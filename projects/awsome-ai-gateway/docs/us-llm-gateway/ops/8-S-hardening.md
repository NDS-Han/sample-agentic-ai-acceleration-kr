...
| 항목          | 조치                                                                                                                                                               |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| admin 콘솔 보호 | 두 가지 — **계정**: Cognito 로그인을 켜고 dev-login 을 끈다([8-L](8-L-admin-login.md), US-12) · **네트워크**: admin-ui·admin-api 를 관리자 IP/VPN 전용으로 좁힌다(아래 "admin 콘솔은 네트워크로 보호"). 둘을 같이 쓰면 이중 보호. |
| 입구 접근제한     | values `inbound-cidrs` 를 **직원 출구 대역**으로 확대하고 **설치용 관리자** `/32` **는 제거**. HTTPS 대신 이 IP제한이 보호막(§0 결정). 절차 = (1)(2) 아래.                                            |
| 허용 모델 목록    | §4에서 등록한 4모델 외 불필요 alias 정리.                                                                                                                                     |
...
Cognito 로그인([8-L](8-L-admin-login.md))을 켜기 전까지 admin-ui 의 로그인은 dev-login 뿐이다 — 역할만 고르면 서명 없는 관리자 토큰이 나온다. 그동안은(켠 뒤에도 이중 보호로) admin 콘솔을 네트워크로 가둔다:
...
> 🔴 admin-ui·admin-api 를 `**0.0.0.0/0` 이나 광범위 대역에 두지 말 것** — dev-login 이 켜져 있으면 **서명 없는 admin 토큰**을 즉시 내주므로 닿는 사람 = admin 이다.
