...
>
> **이 문서를 봐야 하나?** US-10 을 [8-D](8-D-upstream-sync.md) 로 했다면 **건너뛰어도 된다** — 8-D ⑧ 이 바로 이 작업이다.
> **모델을 새로 추가했거나 AWS 단가가 바뀌었을 때** 다시 와서 ② 부터 따라 한다.
...
- **upstream 동기화 직후** — [8-D](8-D-upstream-sync.md) ⑧ 이 바로 이 작업이다(US-10 을 8-D 로 했으면 이미 끝). 마이그레이션이 단가를 글로벌 값으로 다시 넣기 때문이다(2026-09-20 prod 실측 — Sonnet 5 가 $2/$10 로 돌아갔다).
- **AWS 단가가 바뀌었을 때** — ② 에서 표를 먼저 고친다.
- **모델을 새로 등록했을 때**([8-M](8-M-models.md)) — `02` 는 `config.env` 의 단가로 넣는다. 등록 뒤 ③ 의 첫 줄(차이 확인)로 표와 맞는지 본다.
- 처음 설치는 해당 없음 — install-guide §4-2 (C) 가 같은 표의 값을 심는다.
