...
- `17` 은 기존 배포에선 표 4행(web search 결과 크기·결과 수·턴당 검색 수·반복 수)이 전부 `<- change` 다 — 그래서 `--apply` 까지 두 줄(`yes` 입력, 백업 후 values 에 삽입, helm 렌더로 검증). 이미 들어 있으면 `nothing to do` 로 지나간다. ⑦ 의 롤아웃에서 적용되고, **⑦ 을 이미 끝낸 뒤라면 `install-eks.sh` 를 한 번 더** 돌린다. 검색 결과가 다음 턴 입력으로 되돌아와 반복마다 과금되는 구조라, 기본값(60,000자·10개·4회·5회)으로는 검색 질문 1건이 $3 를 넘긴다(2026-09-16 실측 $3.62) — 12,000자·5개·2회·3회면 약 $1.5 이하. 값은 `config.env` 의 `WEB_SEARCH_MAX_*`.
