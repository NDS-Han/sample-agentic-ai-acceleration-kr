...
|---|---|---|---|
| [**IN-02**](ops/8-V-body-logging.md) 2026/09 | Request/response body logging — the gateway writes full bodies to S3 via Firehose (audit · debugging). ⚠️ Stored **unmasked** — privacy review required · collection on/off is the admin `/monitoring` toggle | Optional (when audit is needed) · not in new installs — off by default; tfvars `enable_body_logging` + helm env prepare the infrastructure only | [8-V](ops/8-V-body-logging.md) — `enable-body-logging.sh dev --apply` (terraform sink + helm env) → `/monitoring` toggle ON. The toggle alone cannot turn it on |
